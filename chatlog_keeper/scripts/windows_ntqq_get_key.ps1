...
param(
    ...
    # NOTE: QQ is single-instance. The DEBUG_ONLY_THIS_PROCESS QQ.exe
    # we launch is killed by a running instance's singleton guard before the INT3
    # breakpoint on sqlite3_key_v2 can fire, so extraction silently fails whenever
...
# Saved UTF-8 WITH BOM so PowerShell 5.1+ parses Chinese strings with no re-read
# (mirrors windows_wechat_get_key.ps1). Only force console output to UTF-8 so the
# key / prompt lines render correctly when captured to a transcript. The old
# self-reload block was removed: PS parses the whole file BEFORE executing, so on
# a no-BOM file it died at parse time and never reached the reload; and its
    ...
    public sealed class KeyExtractor
    {
        ...
        private string HandleBreakpoint(uint threadId)
        {
            ...
            try
            {
                ...
                // 只取 整数+控制 寄存器 (R8=key, Rip), 绝不用 CONTEXT_ALL: VectorRegister 声明为
                // ulong[26]=208B 但真实 M128A[26]=416B, CONTEXT_ALL 读写损坏 FPU/SSE → 友机崩
                // (微信调试器同坑实证修复 2026-06-08)。
        ...
        private void SetSingleStep(IntPtr hThread, ref CONTEXT64 ctx, uint threadId)
        {
            ...
            ctx.ContextFlags = Native.CONTEXT_CONTROL; // 只写回 Rip/EFlags 控制寄存器 (不碰 FltSave/Vector)
        ...
        private void ClearTrapFlag(uint threadId)
        {
            ...
            {
                CONTEXT64 ctx = new CONTEXT64();
                ctx.ContextFlags = Native.CONTEXT_CONTROL; // 仅控制寄存器 (清 trap flag), 不用 CONTEXT_ALL 避免 FPU/SSE 损坏
                ctx.FltSave = new byte[512];
                ctx.VectorRegister = new ulong[26];
...
if ($KillExisting) {
    ...
    if ($running.Count -gt 0) {
        ...
        while ((Get-Date) -lt $deadline) {
            if (@(Get-Process -Name 'QQ', 'QQNT' -ErrorAction SilentlyContinue).Count -eq 0) { break }
            Start-Sleep -Milliseconds 250
        }
        $still = @(Get-Process -Name 'QQ', 'QQNT' -ErrorAction SilentlyContinue)
        if ($still.Count -gt 0) {
            Write-Host "警告: QQ 进程未在 5s 内退出, 仍尝试启动调试实例 (可能失败)。" -ForegroundColor Red
        }
        else {
            Write-Host "QQ 已关闭, 即将启动受调试的 QQ 实例 — 请在弹出的窗口登录目标账号。" -ForegroundColor Green
        }
    }
}
